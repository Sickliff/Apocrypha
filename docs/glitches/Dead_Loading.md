...
**Dead Loading** is a bug in all versions of Skyrim that allows you to [duplicate](https://www.apocrypha.wiki/glitches/Duplication) items, [load warp](https://www.apocrypha.wiki/glitches/Load_Warping) through doors, and more generally activate objects from far away. 
...
## Applications 
### Remote Interaction
Many objects such as chairs, levers, and even NPCs can be interacted with from far away by dead loading as long as both saves are in the same cell. This can be used for [sequence breaks](https://www.apocrypha.wiki/glitches/Sequence_Break) or other unintended behavior. 

For more information, see the [remote interaction](https://www.apocrypha.wiki/glitches/Remote_Interaction) article.
...
For other ways to traverse Skyrim quickly, see the [traversal](https://www.apocrypha.wiki/glitches/Traversal) article.

### Item Duplication
This duplication method works because you can pick up the same item multiple times in the loading screen, though this can have varying results depending on the item's [ID number](https://www.apocrypha.wiki/engine/Numeric_ID). Generally, only items that start out as part of the world can be dead loaded, and items that are dropped by the player or spawn in random loot cannot. However, certain artifacts or unique items can be dead loaded even after being picked up, such as Auriel's Bow or the Aetherial Crown. 

For more duplication methods, see the [duplication](https://www.apocrypha.wiki/glitches/Duplication) article.

## Notes 
- Quitting to the main menu while dead loading is active will cause the player to die in the main menu, causing a [crash](https://www.apocrypha.wiki/glitches/Crashes) or [infinite loading screen](https://www.apocrypha.wiki/glitches/Softlocks).
- If affected by a very large Fortify Health enchantment (such as one attained with the use of an [alchemy loop](https://www.apocrypha.wiki/glitches/Alchemy_Loop)), being damaged by a sufficient amount may also activate dead loading.
